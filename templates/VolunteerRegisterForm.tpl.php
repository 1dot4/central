<form class='form-inline'>
    <h2 class='form-heading'>Enter your details as a volunteer</h2>
    <hr><br>
    <div class='form-group'>
        <input type='text' class='form-control' name='name' placeholder='Enter volunteer or NGO name' />
    </div>
    <br><br>
    <div class='form-group'>
        <input type='text' class='form-control' name='phone' placeholder='Enter your contact number' /><br/>
    </div>
    <br><br>
    <div class='form-group'>
        <input type='email' class='form-control' name='email' placeholder='Enter you email id' /><br/>
    </div>
    <br><br>
    <div class='form-group'>
        <input type='password' class='form-control' name='password' placeholder='Enter your password' /><br/>
    </div>
    <br><br>
    <div class='form-group'>
        <input type='password' class='form-control' name='cpassword' placeholder='Confirm your password'><br/>
    </div>
    <br><br>
    <input type='submit' value='Register' class='btn btn-primary'>
    <hr>
    <a href='../register'>< Back</a>
</form>
